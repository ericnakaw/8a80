...
//    se a quantidade nao for menor que 1, entao:
    if ($quantity > 0) {
        ...
        header('Location: orcamento_produto.php?action=quantity_updated&id=' . $id . '&name=' . $name . '&quantity=' . $quantity);
    } else {
        // redirect to product list and tell the user it was added to cart
        header('Location: orcamento_produto.php?action=update_error&id=' . $id . '&name=' . $name . '&quantity=' . $quantity);
    }
}
